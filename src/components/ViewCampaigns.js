  ...
  return (
    <div className="bg-gray-100 border-3 flex flex-wrap justify-center w-full">
      <div className="w-full">
        ...
        <div className="flex gap-4 flex-wrap justify-center w-11/12 mx-auto">
          {!loading &&
            campaigns.length > 0 &&
            campaigns
              .filter((campaign) => {
                const deadline_Days = daysLeft(campaign.deadline);
                return (
                  deadline_Days > 0 && campaign.amountCollected < campaign.target
                );
              })
              .map((campaign) => (
                <CampaignCard
